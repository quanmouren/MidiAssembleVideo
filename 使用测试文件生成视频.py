...
# 判断音符文件夹是否存在
note_folder = "test/sounds"
input_folder = "test\\soundsForMP3"
output_folder = "test\\sounds"
video_size = (540, 960)  # 视频尺寸

if not os.path.exists(note_folder):
    process_audio_folder(input_folder, output_folder, video_size)
elif len(os.listdir(note_folder)) != 88:
    try:
        shutil.rmtree(note_folder)
        process_audio_folder(input_folder, output_folder, video_size)
    except Exception as e:
        print(f"删除文件夹 {note_folder} 或生成视频时出错: {str(e)}")

#生成视频 仅生成30秒用于测试
midi_file_path = "test/input_test.mid"
notes = parse_midi_notes(midi_file_path)
try:
    VideoRendering(notes, video_dir="test/sounds", sustained=0.4, output_video_path="The_video_generated_using_the_test_file.mp4",chord_size_ratio=0.44, start_render_time=0, end_render_time=30)
except Exception as e:
    print("运行错误:", e)
    runwebserver(midi_file_path, "test/sounds")