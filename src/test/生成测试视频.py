...
def create_note_video(audio_path, output_path, size=(320, 240)):
    ...
    if duration is None or duration <= 0:
        print(f"警告: {audio_path} 的音频时长无效，跳过")
        audio.close()
        return
    ...
    background = ColorClip(size=size, color=bg_color, duration=duration).set_fps(10)
    text_clip = ImageClip(text_image).set_duration(duration).set_position('center').set_fps(10)

    final_clip = CompositeVideoClip([background, text_clip], size=size).set_duration(duration)
    final_clip = final_clip.set_audio(audio)
    final_clip = final_clip.set_fps(10)
