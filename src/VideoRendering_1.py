...
def VideoRendering(notes, video_dir="sounds", sustained=0.3, output_video_path="output_video.mp4", 
                   chord_time_offset=0.1,  # 新增：和弦音符时间错开间隔
                   start_render_time=None, end_render_time=None):
    """
    渲染MIDI文件为视频
    notes: 音符列表
    video_dir: 视频目录
    sustained: 音符保持时间（延音）
    chord_time_offset: 和弦音符之间的时间错开间隔（秒）
    ...
    for start_key, group_notes in note_groups.items():
        ...
        base_start_time = group_notes[0]['start_time']  # 基础开始时间
        print(f"处理时间点 {base_start_time:.4f} 的 {num_notes} 个音符")
        ...
        for i, note in enumerate(group_notes):
            ...
            # 计算当前音符的开始时间（主音符不变，和弦音符按偏移量错开）
            current_start_time = base_start_time + (i * chord_time_offset)
...
if __name__ == "__main__":
    ...
    notes = parse_midi_notes("test/input_test.mid")
    ...
    VideoRendering(
        ...
        start_render_time=221,
        end_render_time=222.4
    )